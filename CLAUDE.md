...
- **`game-performance-booster`** — dado el id de un juego, audita su componente canvas y su reproductor y aplica las optimizaciones del patrón Frogger (spec 12): offscreen canvas para fondo estático y scanlines retro, HUD con `useRef`+`textContent` en lugar de `useState`, y `shadowBlur` agrupado en un único bloque por frame. Implementa código y verifica con `pnpm lint`/`pnpm build`. Mantiene bitácora en `.claude/agents/game-performance-booster-memory.md` y registro de cobertura en `references/game-performance.md`.
- **`security-auditor`** — audita la seguridad de toda la plataforma (BD Supabase + app Next.js) contra el checklist del spec 14. En la app aplica y verifica correcciones (headers HTTP, `PASSWORD_REGEX`, proxy de rutas). En la BD audita en solo lectura (RLS, políticas, funciones `SECURITY DEFINER`, Auth settings) y **propone el SQL sin aplicarlo** — el humano lo ejecuta tras revisarlo. Guarda el log de cada auditoría en `references/security/security-check-log.md` y bitácora en `.claude/agents/security-auditor-memory.md`.
