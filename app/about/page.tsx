...
function HighlightIcon({ kind }: { kind: string }) {
  ...
  if (kind === "HEART") return (
    <svg className="hl-icon" viewBox="0 0 16 16" width="36" height="36"><g fill={C}>
          ...
          <form className={"contact-form" + (shake ? " shake" : "")} onSubmit={onSubmit}>
            {!sent ? (
              <>
                ...
                {apiError && (
                  <div style={{ color: "var(--magenta)", fontFamily: "var(--mono)", fontSize: 13, marginBottom: 12, padding: "8px 10px", border: "1px solid var(--magenta)", background: "rgba(255,0,110,0.08)" }}>
                    {apiError}
                  </div>
                )}
                <button className="btn xl press" type="submit" disabled={submitting} style={{ width: "100%" }}>
                ...
                <div className="term-body">
                  ...
                  <div style={{ marginTop: 18 }}>
                    <button
                      className="btn ghost"
                      type="button"
                      onClick={() => { setSent(null); setForm({ name: "", email: "", msg: "" }); setApiError(null); }}
                    >
                      ENVIAR OTRO MENSAJE
                    </button>
                  </div>
